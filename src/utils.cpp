...
std::vector<Circle> generate_circles(int num_circles, int width, int height) {
    ...
    std::uniform_real_distribution<float> z_dist(0.0f, 1000.0f);
    std::uniform_real_distribution<float> r_dist(10.0f, std::min(width, height) / 10.0f);
    std::uniform_int_distribution<int> c_dist(0, 255);
    std::uniform_real_distribution<float> a_dist(0.3f, 0.8f);
